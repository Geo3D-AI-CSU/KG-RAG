...
class KnowledgeGraphRAG:
    ...
    def retrieve_subgraph(self, question, triplets, node_embeddings, relation_embeddings, max_hops=3):
        ...
        for triplet in triplets:
            ...
            candidate_triplets.append((triplet, score))  # 保存三元组及其评分

        selected_triplets = [triplet for triplet, score in candidate_triplets if score > thresholds[0]]
        # 如果候选三元组数量小于10，按评分排序并扩展
        if len(selected_triplets) < 12:
            # 按评分排序
            candidate_triplets.sort(key=lambda x: x[1], reverse=True)
            # 选取评分最高的10个
            selected_triplets = [triplet for triplet, score in candidate_triplets[:12]]
